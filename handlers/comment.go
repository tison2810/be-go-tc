...
func GetPostComment(c *fiber.Ctx) error {
	...
	if err := database.DB.Db.Where("post_id = ? AND is_deleted IN (?)", id, []string{"active", "similar"}).Find(&comments).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func UpdateComment(c *fiber.Ctx) error {
	...
	// Tìm comment
	comment := new(models.Comment)
	if err := database.DB.Db.Where("id = ? AND is_deleted IN (?)", id, []string{"active", "similar"}).First(&comment).Error; err != nil {
...
func UpdateCommentFormData(c *fiber.Ctx) error {
	...
	// Tìm comment
	comment := new(models.Comment)
	if err := database.DB.Db.Where("id = ? AND is_deleted IN (?)", commentID, []string{"active", "similar"}).First(comment).Error; err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": "Comment not found or already deleted",
		})
	}
